...
if page == "📊 Dashboard":
    ...
    if not df.empty:
        ...
        m1.metric("Team Avg Utilization", f"{avg_util:.0f}%", delta=f"{avg_util-100:.0f}%" if avg_util > 100 else None)
        m2.metric("ACP Utilization", f"{acp_util:.0f}%")
        m3.metric("CP/SCP Utilization", f"{cp_util:.0f}%")
        m4.metric("ACE/CE/SCE Util", f"{ce_util:.0f}%")
        st.divider()

        col_l, col_r = st.columns(2)
        
        # LEFT: MASTER PROGRAM TABLE
        with col_l:
        ...
        with col_r:
            ...
            emp_view_df = df.copy()
            # Always hide R+I from list, unless toggle is ON which allows them to exist in data context?
            # User said: "Allocations by Employee view does not include any employees with roles that begin with R+I"
