...
if page == "📊 Dashboard":
    ...
    if not df.empty:
        ...
        # 1. Team Avg (Uses utils.TEAM_ROLES list: ACP, CP, SCP, ACE, CE, SCE)
        ...
        # 3. CP / SCP
        cp_util, cp_alloc, cp_cap = utils.calculate_group_utilization(df, ['CP', 'SCP'])
        ...
        # Restore the "Container" look for the main metric
        with m1:
            with st.container(border=True):
                st.metric(
        ...
        # PREPARE DATA FOR TABLES BASED ON TOGGLE
        col_l, col_r = st.columns(2)
        
        with col_l:
            # TOGGLE ABOVE CHART
            include_ri = st.toggle("Include R+I Roles?", value=True)
            ...
            if prog_cols:
                dynamic_margin = utils.calculate_margin(analysis_df, st.session_state.program_mrr)
                master_data = []
                for p in prog_cols:
                ...
                if not master_df.empty:
                    ...
                    st.dataframe(
                        ...
                        column_config={
                            ...
                            "Program MRR": st.column_config.NumberColumn("Program MRR", format="$%d"),
        ...
        with col_r:
            # PUSH DOWN TO ALIGN WITH TABLE
            st.write("") 
            st.write("") 
            st.write("") 
            # Note: The spacer above approximates the height of the toggle so headers align
            st.subheader("Allocations by Employee")
            
            # Using the SAME filtered `analysis_df` from the left column so the toggle controls both
